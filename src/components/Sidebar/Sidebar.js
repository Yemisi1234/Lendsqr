...
import { SuitCase, UsersIcon, GroupUsers, MoneyBag, HandShake, PiggyBank, Savings, UserCheck, UserTimes, Bank, CoinStack, Transaction, Services, UserCog, Settlements, Chart, Preferences, Pricing, Wheel, Logout, Clipboard, HomeIcon } from '../../images/icons';
    ...
    return (
        <div className="sidebar">
            <div className="sidebar-container">
                <h3 className="sidebar-header"><SuitCase className="sidebar-header-icon" /><span className="sidebar-header-text">Switch Organization</span><MdOutlineKeyboardArrowDown className="switch-dropdown" /></h3>
                <h3 className="sidebar-header"><HomeIcon className="sidebar-header-icon" /><span className="sidebar-header-text">Dashboard</span></h3>
            </div>
            <div className="side-items-container">
                <h4 className="side-items-header">CUSTOMERS</h4>
                <ul className="side-items">
                    {navItems.map(item => (<li className="side-item"><Link to="/" className="side-link">{item.icon} <span className="side-text">{item.name}</span></Link></li>))}
                </ul>
                <h4 className="side-items-header">BUSINESSES</h4>
                <ul className="side-items">
                    {navItems2.map(item => (<li className="side-item"><Link to="/" className="side-link">{item.icon} <span className="side-text">{item.name}</span></Link></li>))}
                </ul>
                <h4 className="side-items-header">SETTINGS</h4>
                <ul className="side-items">
                    {navItems3.map(item => (<li className="side-item"><Link to="/" className="side-link">{item.icon} <span className="side-text">{item.name}</span></Link></li>))}
                </ul>
            </div>
            <ul>
